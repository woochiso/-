...
import java.io.File
@Composable fun DebateTrainingScreen(vm:DebateTrainingViewModel,onAuthExpired:()->Unit){val s by vm.state.collectAsStateWithLifecycle();val ctx=androidx.compose.ui.platform.LocalContext.current;val p=remember{MediaPlayer()};val cache=remember{mutableMapOf<String,File>()};var playing by remember{mutableStateOf(false)};var active by remember{mutableStateOf<String?>(null)};var voiceBusy by remember{mutableStateOf(false)}
...
@Composable private fun DebateRoom(s:DebateUiState,vm:DebateTrainingViewModel,onAuthExpired:()->Unit,voiceBusy:Boolean,setVoiceBusy:(Boolean)->Unit,stopAudio:()->Unit,playing:Boolean,audio:(String,String)->Unit){val list=rememberLazyListState();val userReadingHistory by remember{derivedStateOf{val last=list.layoutInfo.visibleItemsInfo.lastOrNull()?.index?:-1;list.isScrollInProgress&&last<s.messages.lastIndex-1}};var keepLatestVisible by remember{mutableStateOf(true)};LaunchedEffect(userReadingHistory){if(userReadingHistory)keepLatestVisible=false else if(!list.canScrollForward)keepLatestVisible=true};LaunchedEffect(s.messages.size){if(keepLatestVisible&&s.messages.isNotEmpty())list.animateScrollToItem(s.messages.lastIndex)};Column(Modifier.fillMaxSize().imePadding().padding(horizontal=16.dp)){Text(s.plan?.topic.orEmpty(),fontWeight=FontWeight.Bold);Text("내 입장: ${s.plan?.userSide} · AI: ${s.plan?.debaterSide} · 내 발언 ${s.turns}/${s.config?.maxTurns?:12}",style=MaterialTheme.typography.bodySmall,color=FreshTextVariant);LazyColumn(Modifier.weight(1f),state=list,verticalArrangement=Arrangement.spacedBy(9.dp)){items(s.messages){m->Row(Modifier.fillMaxWidth(),horizontalArrangement=if(m.role=="user")Arrangement.End else Arrangement.Start){Card(Modifier.fillMaxWidth(.84f),colors=CardDefaults.cardColors(if(m.role=="user")FreshLightIndigoContainer else Color.White),border=BorderStroke(1.dp,FreshOutline)){Column(Modifier.padding(12.dp)){Text(if(m.role=="user")"👤 나" else if(m.role=="moderator")"🎙️ AI 진행자" else "🤖 AI 토론자",fontWeight=FontWeight.Bold);Text(m.text);m.ttsToken?.let{TextButton({audio(it,m.role)}){Icon(if(playing)Icons.Default.Stop else Icons.Default.PlayArrow,null);Text(if(playing)"음성 멈춤" else "의견 듣기")}}}}}}};s.error?.let{Err(it,vm::retry)};if(s.busy)Text("AI가 반론을 생각하고 있어요...",color=FreshDeepIndigo);OutlinedTextField(s.input,vm::input,Modifier.fillMaxWidth(),enabled=!voiceBusy&&!s.busy,placeholder={Text("내 주장과 근거를 입력하세요")});VoiceRecordingPanel(enabled=!s.busy,currentText="",onTranscribed={text->if(text.isNotBlank()){vm.input(text);vm.send()}},onAuthExpired=onAuthExpired,autoTranscribeOnStop=true,compact=true,onRecordingStarted=stopAudio,onBusyChanged=setVoiceBusy);Row{Button(vm::send,Modifier.weight(1f),enabled=s.input.isNotBlank()&&!s.busy&&!voiceBusy&&s.turns<12){Icon(Icons.Default.Send,null);Text(" 보내기")};TextButton(vm::finish,enabled=s.turns>0&&!s.busy&&!voiceBusy){Text("■ 토론 마무리")}}}}
