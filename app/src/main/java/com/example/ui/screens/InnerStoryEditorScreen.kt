...
@Composable fun InnerStoryEditorScreen(initial:StoryDto?=null,onCancel:()->Unit,onCreate:(StoryWriteRequest)->Unit,onUpdate:(StoryUpdateRequest)->Unit,modifier:Modifier=Modifier){
 var title by remember(initial){mutableStateOf(initial?.title.orEmpty())};var content by remember(initial){mutableStateOf(initial?.content.orEmpty())};var period by remember(initial){mutableStateOf(initial?.period.orEmpty())};var age by remember(initial){mutableStateOf(initial?.age?.toString().orEmpty())};var year by remember(initial){mutableStateOf(initial?.year?.toString().orEmpty())};var category by remember(initial){mutableStateOf(categoryLabel(initial?.eventCategory))};var person by remember(initial){mutableStateOf(personLabel(initial?.relatedPerson))};var importance by remember(initial){mutableIntStateOf(initial?.importance?:1)};var impact by remember(initial){mutableIntStateOf(initial?.currentImpact?:1)};var status by remember(initial){mutableStateOf(statusLabel(initial?.currentStatus))}
 val valid=title.isNotBlank()&&title.length<=150&&content.isNotBlank()&&content.length<=5000&&period.isNotBlank()&&period.length<=100&&(age.isBlank()||age.toIntOrNull() in 0..120)&&(year.isBlank()||year.toIntOrNull() in 1900..Year.now().value)
 LazyColumn(modifier.fillMaxSize(),contentPadding=PaddingValues(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
  ...
  item{Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){OutlinedTextField(age,{age=it.filter(Char::isDigit).take(3)},label={Text("당시 나이 (선택)")},keyboardOptions=KeyboardOptions(keyboardType=KeyboardType.Number),modifier=Modifier.weight(1f));OutlinedTextField(year,{year=it.filter(Char::isDigit).take(4)},label={Text("사건 연도 (선택)")},keyboardOptions=KeyboardOptions(keyboardType=KeyboardType.Number),modifier=Modifier.weight(1f))}}
