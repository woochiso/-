...
enum class CounselingStage{MODE,START,TODAY,STORIES,CHAT,HISTORY}
data class CounselingUiState(
    val loading:Boolean=false,val sending:Boolean=false,val stage:CounselingStage=CounselingStage.MODE,
    ...
    val deletingSessionId:Long?=null,val notice:String?=null,val voiceMode:Boolean=false,
    val speechLoading:Boolean=false,val speechBytes:ByteArray?=null,val speechMessageId:Long?=null,val speechError:String?=null,
...
class CounselingViewModel(application:Application):AndroidViewModel(application){
    ...
    fun startToday(){_state.value=_state.value.copy(stage=CounselingStage.CHAT,sessionId=null,messages=listOf(greeting("오늘 기록한 감정을 참고해서 이야기해볼까요? 어떤 감정부터 나누고 싶으세요?")),includeEmotion=true,selectedStory=null,aiError=null,crisis=null)}
    ...
    fun navigateBack():Boolean{
        when(_state.value.stage){
            ...
            CounselingStage.TODAY,CounselingStage.STORIES,CounselingStage.CHAT->_state.value=_state.value.copy(stage=if(_state.value.voiceMode)CounselingStage.MODE else CounselingStage.START,voiceMode=false)
            CounselingStage.START,CounselingStage.HISTORY->newCounseling()
        }
        return true
    }
    ...
    fun sendRecognized(text:String){if(text.isBlank()||_state.value.sending)return;setInput(text);send()}
    fun requestSpeech(message:CounselingMessageDto){
        val sessionId=_state.value.sessionId?:return
        if(message.messageId<=0||_state.value.speechLoading||_state.value.speechMessageId==message.messageId)return
    ...
    private fun execute(request:CounselingMessageRequest){viewModelScope.launch{_state.value=_state.value.copy(sending=true,aiError=null,crisis=null);when(val result=repository.send(request)){is EmotionServerResult.Success->{val id=result.value.sessionId?:request.sessionId;if(id==null){_state.value=_state.value.copy(sending=false,aiError="상담 응답을 확인하지 못했습니다.",crisis=result.value.crisis);return@launch};pending=request.copy(sessionId=id);when(val detail=repository.detail(id)){is EmotionServerResult.Success->{val hasAssistant=detail.value.messages.lastOrNull()?.role=="ASSISTANT";if(hasAssistant)pending=null;_state.value=_state.value.copy(sending=false,sessionId=id,messages=detail.value.messages,aiError=if(hasAssistant)null else result.value.message?:"답변을 불러오지 못했습니다.",crisis=result.value.crisis)};EmotionServerResult.Unauthorized->_state.value=_state.value.copy(sending=false,requiresLogin=true);is EmotionServerResult.Error->_state.value=_state.value.copy(sending=false,sessionId=id,aiError=detail.message,crisis=result.value.crisis)}};EmotionServerResult.Unauthorized->_state.value=_state.value.copy(sending=false,requiresLogin=true);is EmotionServerResult.Error->_state.value=_state.value.copy(sending=false,aiError=result.message)}}}
