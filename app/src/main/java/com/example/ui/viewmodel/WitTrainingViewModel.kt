...
data class WitTrainingUiState(val loading:Boolean=true,val sending:Boolean=false,val speechLoading:Boolean=false,val config:WitConfigResponse?=null,val stage:WitStage=WitStage.CHOOSE,val category:String="",val categoryTitle:String="",val customSituation:String="",val voice:String="DEFAULT",val practiceId:String?=null,val categoryDescription:String="",val context:String="",val scenario:String="",val characterLine:String="",val inputText:String="",val result:WitActionResponse?=null,val speechBytes:ByteArray?=null,val speechToken:String?=null,val speechVoice:String?=null,val error:String?=null,val speechError:String?=null,val requiresLogin:Boolean=false)
class WitTrainingViewModel(app:Application):AndroidViewModel(app){
    ...
    fun custom(v:String){if(v.length<=300)_state.value=_state.value.copy(customSituation=v)}
    fun voice(v:String){
        val token=_state.value.speechToken
        _state.value=_state.value.copy(voice=v,speechBytes=null,speechVoice=null,speechError=null)
        token?.takeIf(String::isNotBlank)?.let(::speech)
    }
    fun replaySpeech(){_state.value.speechToken?.takeIf(String::isNotBlank)?.let(::speech)}
    fun text(v:String){if(v.length<=2000)_state.value=_state.value.copy(inputText=v,error=null)}
    ...
    private fun speech(token:String){speechJob?.cancel();val requestedVoice=_state.value.voice;speechJob=viewModelScope.launch{_state.value=_state.value.copy(speechLoading=true,speechError=null,speechBytes=null,speechToken=token,speechVoice=null);when(val r=repo.speech(token,requestedVoice)){is EmotionServerResult.Success->if(_state.value.voice==requestedVoice&&_state.value.speechToken==token)_state.value=_state.value.copy(speechLoading=false,speechBytes=r.value,speechVoice=requestedVoice);EmotionServerResult.Unauthorized->_state.value=_state.value.copy(speechLoading=false,requiresLogin=true);is EmotionServerResult.Error->if(_state.value.voice==requestedVoice&&_state.value.speechToken==token)_state.value=_state.value.copy(speechLoading=false,speechError=r.message)}}}
}
