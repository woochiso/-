...
class WitTrainingRepository(private val api:ApiService,private val tokens:TokenManager){
    ...
    suspend fun speech(token:String,voice:String):EmotionServerResult<ByteArray>{
        val a=auth()?:return EmotionServerResult.Unauthorized
        return try{
            if(BuildConfig.DEBUG)Log.d("WIT_TTS","request start voice=$voice")
            val r=api.getWitSpeech(a,WitTtsRequest(token,voice));val b=r.body()?.bytes();val t=r.headers()["Content-Type"].orEmpty().lowercase()
            if(BuildConfig.DEBUG)Log.d("WIT_TTS","response status=${r.code()} contentType=${t.ifBlank{"missing"}} audioReceived=${b!=null&&b.size>12}")
            when{r.isSuccessful&&b!=null&&b.size>12&&(t.startsWith("audio/")||t=="application/octet-stream")->EmotionServerResult.Success(b);r.code()==401->EmotionServerResult.Unauthorized;else->EmotionServerResult.Error(r.code(),message(r.errorBody()?.string(),"음성을 불러오지 못했어요."))}
        }catch(e:Exception){if(BuildConfig.DEBUG)Log.e("WIT_TTS","request failed type=${e.javaClass.simpleName}");EmotionServerResult.Error(0,"음성을 불러오지 못했어요.")}
    }
